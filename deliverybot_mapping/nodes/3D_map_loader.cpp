#include "3D_map_loader.h"

void MapLoader::sub_callback(const std_msgs::String msg){
    ros::NodeHandle nh("~");
    ...
    pcd_file_path = msg.data;
    pcd_file_path.erase (pcd_file_path.end()-5,pcd_file_path.end());
    pcd_file_path.append(".pcd");
    while(int(file_list_.size())!=0) file_list_.pop_back();
    ...
    if (out_msg.width != 0) {
        ...
        ROS_INFO("finish loading 3D point cloud map !!!");
	}
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub = nh.subscribe("/map_path", 1000, &MapLoader::sub_callback, &map_loader);
